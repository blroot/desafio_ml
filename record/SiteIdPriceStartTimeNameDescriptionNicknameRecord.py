...
class SiteIdPriceStartTimeNameDescriptionNicknameRecord(Record):
    ...
    def retrieve_item(self, ml_api: MLApi) -> List[Tuple] or None:
        ...
        item_async = ml_api.items.get(element_id=self._item_id(),
    ...
    def retrieve_all_details(self, ml_api: MLApi) -> List[Tuple] or None:
        item = ml_api.items.get_from_cache(element_id=self._item_id())
        if item['code'] != 200:
            self.cancel_pipeline()
            return
        ...
        currency_async = ml_api.currencies.get(element_id=item['body']['currency_id'],
                                               extra_args='attributes=description')
        category_async = ml_api.categories.get(element_id=item['body']['category_id'],
                                               extra_args='attributes=name')
        user_async = ml_api.users.get(element_id=item['body']['seller_id'],
                                      extra_args='attributes=nickname')

        return [currency_async, category_async, user_async]

    def end_pipeline(self, ml_api: MLApi):
        self.description = ml_api.currencies.get_from_cache(element_id=self.currency_id)['description']
        self.name = ml_api.categories.get_from_cache(element_id=self.category_id)['name']
        self.nickname = ml_api.users.get_from_cache(element_id=self.user_id)['nickname']
